...
for ((w=0; w<DECODE_WORKERS; w++)); do
  ...
  DYN_SYSTEM_PORT="${SYSTEM_PORT}" \
    python3 -m dynamo.sglang \
      --model-path "$MODEL_PATH" \
      --served-model-name "$SERVED_MODEL_NAME" \
      --tensor-parallel-size "4" --pipeline-parallel-size "1" --data-parallel-size "1" --kv-cache-dtype "fp8_e5m2" --max-running-requests "2" --expert-parallel-size "1" --moe-dense-tp-size "1" --cuda-graph-bs 1 2 --disaggregation-mode decode \
      --host "0.0.0.0" \
      --enable-metrics &
done
wait
