...
func (r *StatefulSetReconciler) reconcileStatefulSet(
	...
	revisionHashEqual := newSts.Labels[fmt.Sprintf(workloadsv1alpha1.RoleRevisionKeyFmt, role.Name)] == oldSts.Labels[fmt.Sprintf(workloadsv1alpha1.RoleRevisionKeyFmt, role.Name)]
	if !revisionHashEqual {
		logger.Info(fmt.Sprintf("lws hash not equal, old: %s, new: %s", oldSts.Labels[fmt.Sprintf(workloadsv1alpha1.RoleRevisionKeyFmt, role.Name)], newSts.Labels[fmt.Sprintf(workloadsv1alpha1.RoleRevisionKeyFmt, role.Name)]))
	}

	stsUpdated := !semanticallyEqual
...
func (r *StatefulSetReconciler) constructStatefulSetApplyConfiguration(
	...
	stsLabel[fmt.Sprintf(workloadsv1alpha1.RoleRevisionKeyFmt, role.Name)] = revisionKey
